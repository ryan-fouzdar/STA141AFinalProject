...
feedback_counts <- table(feedback_types)
average_contrast <- (contrast_left + contrast_right) / 2
...
for (session_num in 1:18) {
  for (trial_num in 1:length(session[[session_num]]$spks)) {
    neurons <- get_num_neurons(session[[session_num]], trial_num)
    if (!is.na(neurons) && is.numeric(neurons)) {
      num_neurons <- c(num_neurons, neurons) 
    }
  }
}
...
set.seed(123)  
kmeansresult <- kmeans(num_neurons, centers = k)

# Print the cluster assignments
print(kmeansresult$cluster)

cluster_data <- data.frame(Trial = 1:length(num_neurons), Cluster = kmeansresult$cluster)
...
for (i in 1:18) { #creating a data frame
  ...
  data <- bind_rows(data, trial_data)
}
...
model <- glm(success ~ contrast_left + contrast_right, data = train_data, family = binomial) #moddel
predictions <- predict(model, newdata = test_data, type = "response") #predictions
print(model)
...
test_features1 <- data.frame(contrast_left = test_contrast_left1, contrast_right = test_contrast_right1)
test_features2 <- data.frame(contrast_left = test_contrast_left2, contrast_right = test_contrast_right2)

#Predict the model
test_predictions1 <- predict(model, newdata = test_features1, type = "response")
test_predictions2 <- predict(model, newdata = test_features2, type = "response")
true_labels1 <- test_data1$feedback_type
true_labels2 <- test_data2$feedback_type
...
fp1 <- confusion_matrix1["-1", "1"]
fn1 <- confusion_matrix1["-1", "1"]
...
fp2 <- confusion_matrix1["-1", "1"]
fn2 <- confusion_matrix2["-1", "1"]
